...
export default function AdminDashboard() {
  ...
  const [testType, setTestType] = React.useState<'unit' | 'integration' | 'e2e' | 'performance'>(
    'unit',
  );
  ...
  const [selectedK8sManifest, setSelectedK8sManifest] = React.useState<
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-dark-bg text-dark-text flex items-center justify-center">
        <div className="text-xs text-dark-muted animate-pulse">
          Loading SwiggyZone Admin dashboard...
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-dark-bg text-dark-text pb-24 max-w-5xl mx-auto p-4 md:p-8 space-y-8 animate-fadeIn">
      ...
      <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-dark-surface/50 border border-dark-border p-6 rounded-3xl backdrop-blur-md">
        <div className="space-y-1">
          ...
          <p className="text-xs text-dark-muted">
            Manage system users, listings, platform commission distributions, and logs.
          </p>
        </div>
      </header>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          {
            label: 'Gross Volume',
            value: `₹${stats?.grossSales?.toFixed(2) || '0.00'}`,
            desc: 'Delivered orders sum',
            icon: TrendingUp,
            color: 'text-green-500 bg-green-500/10 border-green-500/10',
          },
          {
            label: 'Platform Revenue',
            value: `₹${stats?.platformCommission?.toFixed(2) || '0.00'}`,
            desc: '10% platform commission',
            icon: Percent,
          ...
          },
          {
            ...
            desc: 'Registered customer nodes',
            icon: UserIcon,
        ...
        <div className="space-y-4 animate-fadeIn">
          ...
          <div className="bg-dark-surface border border-dark-border rounded-2xl overflow-hidden divide-y divide-dark-border/40">
            {users.map((item) => (
              <div
                key={item.id}
                className="p-4 flex justify-between items-center text-xs flex-wrap gap-4"
              >
                <div className="space-y-1">
            ...
            {restaurants.map((rest) => (
              ...
              >
                ...
                <div className="flex items-center gap-4">
                  ...
                  <Button
                    size="sm"
                    variant={rest.isActive ? 'secondary' : 'primary'}
            ...
            {logs.map((log) => (
              ...
              >
                ...
                <div className="text-right space-y-1">
                  ...
                  <div className="text-[9px] text-dark-muted">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <Card className="p-6 border border-dark-border/40 bg-dark-surface/30 flex flex-col justify-between h-40">
              <div className="flex justify-between items-center">
                ...
                <span className="bg-red-500/10 text-red-500 text-[9px] font-black uppercase px-2 py-0.5 rounded">
                  AI Scan Live
                </span>
              </div>
              <div className="flex items-center gap-4 py-2">
                <div className="text-4xl font-black text-white">{fraudStats?.riskIndex || 0}%</div>
                <div className="space-y-0.5">
                  <div className="text-xs font-black uppercase text-dark-text">
                    {fraudStats?.riskIndex > 85
                      ? '🚨 CRITICAL RISK'
                      : fraudStats?.riskIndex > 60
                        ? '⚠️ HIGH THREAT'
                        : '🟢 MODERATE THREAT'}
                  </div>
                  <p className="text-[10px] text-dark-muted">
                    Aggregated real-time platform risk ratio
                  </p>
                </div>
              </div>
            ...
            <Card className="p-6 border border-dark-border/40 bg-dark-surface/30 flex flex-col justify-between h-40">
              <div className="flex justify-between items-center">
                ...
                <div className="p-1 rounded bg-brand-saffron/10 text-brand-saffron">
              ...
              <div>
                ...
                <p className="text-[10px] text-dark-muted">
                  API requests and checkout sessions scanned in last 24h
                </p>
              </div>
            ...
            <Card className="p-6 border border-dark-border/40 bg-dark-surface/30 flex flex-col justify-between h-40">
              ...
              <div>
                <h3 className="text-2xl font-black text-green-500">
                  ₹{fraudStats?.savingsSaved?.toFixed(2) || '0.00'}
                </h3>
                <p className="text-[10px] text-dark-muted">
                  Savings saved by automatically rejecting bad coupon attempts
                </p>
              </div>
              <div className="text-[10px] text-dark-muted">
                Blocked Profiles:{' '}
                <span className="text-white font-bold">{fraudStats?.blockedCount || 0} users</span>
              </div>
            </Card>
          </div>
            ...
            <div className="lg:col-span-1 space-y-6">
              <Card glass className="p-5 border border-dark-border/40 space-y-4">
                ...
                <p className="text-[11px] text-dark-muted">
                  Trigger automated fraud attacks on our rules engine to test real-time alerting and
                  user mitigation systems.
                </p>
              ...
              <Card className="p-4 border border-dark-border/40 bg-dark-surface/20 text-[11px] space-y-3">
                ...
                <div className="space-y-2 text-dark-muted">
                  <div>
                    <strong>Coupon Abuse:</strong> Triggered when identical card signatures make
                    checkouts using different signup emails.
                  </div>
                  <div>
                    <strong>Fake Reviews:</strong> Triggered by submitting positive reviews under 3
                    seconds from delivery, or repeating phrase structures.
                  </div>
                  <div>
                    <strong>Payment Fraud:</strong> Triggered by velocity constraints (exceeding 3
                    charge attempts with different cards).
                  </div>
                  <div>
                    <strong>Bot Activity:</strong> Headless browsers executing actions with zero
                    touch movements or missing user-agents.
                  </div>
                  <div>
                    <strong>Duplicate Accounts:</strong> Identified using shared audio/canvas
                    fingerprints + identical phone nodes.
                  </div>
                </div>
              </Card>
            </div>
              ...
              {fraudLoading ? (
                <div className="space-y-4">
                  {[1, 2].map((i) => (
                    <div
                      key={i}
                      className="bg-dark-surface border border-dark-border h-24 rounded-2xl animate-pulse"
                    />
                  ))}
                </div>
                      ...
                      >
                        <div className="flex justify-between items-start gap-4 flex-wrap">
                          <div className="space-y-1">
                            ...
                            <h4 className="font-extrabold text-sm text-dark-text pt-1">
                              Target Account: {alert.userEmail}
                            </h4>
                            <p className="text-xs text-dark-muted pt-0.5 leading-relaxed">
                              {alert.description}
                            </p>
                          </div>
                ...
                <div className="text-center py-16 border border-dashed border-dark-border rounded-3xl bg-dark-surface/5 flex flex-col items-center justify-center gap-2">
                  ...
                  <span className="text-xs text-dark-muted">
                    Platform integrity is secure. Trigger simulations on the left to test security
                    systems.
                  </span>
                </div>
              )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            ...
            <Card className="p-5 border border-dark-border/40 bg-dark-surface/30 flex flex-col justify-between h-36">
              ...
              <div className="py-1">
                ...
                <p className="text-[10px] text-dark-muted mt-1">
                  Prisma PostgreSQL engine ping status
                </p>
              </div>
              <span className="text-[9.5px] text-green-500 font-semibold">
                ⚡ PostgreSQL 16 ready
              </span>
            </Card>
            ...
            <Card className="p-5 border border-dark-border/40 bg-dark-surface/30 flex flex-col justify-between h-36">
              ...
              <div className="py-1">
                <span className="text-3xl font-black text-white">
                  {securityHealth?.cpuPercent || 0}%
                </span>
                <p className="text-[10px] text-dark-muted mt-1">
                  Uptime thread polling CPU core load
                </p>
              </div>
            ...
            <Card className="p-5 border border-dark-border/40 bg-dark-surface/30 flex flex-col justify-between h-36">
              ...
              <div className="py-1">
                ...
                <span className="text-xs text-dark-muted ml-1.5">
                  / {securityHealth?.memory?.total || 0} MB
                </span>
                <p className="text-[10px] text-dark-muted mt-1">
                  V8 virtual machine active garbage memory
                </p>
              </div>
            ...
            <Card className="p-5 border border-dark-border/40 bg-dark-surface/30 flex flex-col justify-between h-36">
              ...
              <div className="py-1">
                ...
                <p className="text-[10px] text-dark-muted mt-1">
                  Total process execution running timeline
                </p>
              </div>
              <span className="text-[9.5px] text-dark-muted">
                Service Instance: active-worker-1
              </span>
            </Card>
          </div>
                  ...
                  {[
                    {
                      ...
                      label: '🛡️ Content-Security-Policy',
                      desc: 'Blocks injection and unauthorized assets load',
                    },
                    {
                      key: 'xFrameOptions',
                      label: '🔲 X-Frame-Options (SAMEORIGIN)',
                      desc: 'Prevents clickjacking frame overlays',
                    },
                    {
                      key: 'xXSSProtection',
                      label: '🚫 X-XSS-Protection (1; mode=block)',
                      desc: 'Prevents reflected script triggers',
                    },
                    {
                    ...
                    },
                    {
                      ...
                      desc: 'Blocks mime-sniffing script execution',
                    },
                  ].map((header) => {
            ...
            <div className="space-y-6">
              <Card glass className="p-5 border border-dark-border/40 space-y-4">
                <div className="flex items-center gap-2 border-b border-dark-border pb-3">
                  ...
                  <h3 className="text-xs font-black uppercase tracking-wider text-dark-text">
                    AES-256-GCM Encryption Sandbox
                  </h3>
                </div>
                <p className="text-[11px] text-dark-muted">
                  Enter sensitive credentials (e.g., Credit Card numbers or private API keys) to
                  test key encryption strength and verify decryption logs.
                </p>
                ...
                {cipherText && (
                  <div className="space-y-3 pt-3 border-t border-t-dark-border/60">
                    ...
                    <div className="bg-dark-bg p-3.5 rounded-xl border border-dark-border/40 font-mono text-[9px] space-y-2 text-dark-muted">
                      <div>
                        <span className="text-brand-saffron font-bold block mb-0.5">
                          Ciphertext (AES hex):
                        </span>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <Card glass className="p-5 border border-dark-border/40 space-y-4">
              <div className="flex items-center gap-2 border-b border-dark-border pb-3">
                <Sparkles className="w-4 h-4 text-brand-saffron" />
                <h3 className="text-xs font-black uppercase tracking-wider text-dark-text">
                  Automated Test Suites Runner
                </h3>
              </div>
              <p className="text-[11px] text-dark-muted">
                Execute automated Jest unit, integration, and Cypress end-to-end user journey tests
                against active sandbox services.
              </p>
              ...
              <Button
                className="w-full text-xs h-9 flex items-center justify-center gap-2"
              ...
              <div className="bg-black/80 border border-dark-border rounded-2xl p-4 font-mono text-[9.5px] text-green-400 h-64 overflow-y-auto scrollbar-none space-y-1.5">
                ...
                {testLogs.length === 0 && (
                  <div className="text-dark-muted italic">
                    Console ready. Click "Execute" above.
                  </div>
                )}
              </div>
            </Card>
            ...
            <Card glass className="p-5 border border-dark-border/40 space-y-4">
              <div className="flex items-center gap-2 border-b border-dark-border pb-3">
                <AlertTriangle className="w-4 h-4 text-brand-saffron animate-pulse" />
                <h3 className="text-xs font-black uppercase tracking-wider text-dark-text">
                  Database Load Stress Tester
                </h3>
              </div>
              <p className="text-[11px] text-dark-muted">
                Execute concurrent API stress tests against backend database engines to compute
              ...
              {loadResults ? (
                <div className="space-y-4 pt-3 border-t border-dark-border/60 animate-fadeIn">
                  <div className="grid grid-cols-3 gap-2.5 text-center text-xs">
                    <div className="bg-dark-bg/60 p-2.5 rounded-xl border border-dark-border/40">
                      ...
                      <div className="text-[8px] text-dark-muted uppercase font-bold">
                        Throughput
                      </div>
                    </div>
                    <div className="bg-dark-bg/60 p-2.5 rounded-xl border border-dark-border/40">
                      <div className="text-sm font-black text-white">
                        {loadResults.avgLatencyMs} ms
                      </div>
                      <div className="text-[8px] text-dark-muted uppercase font-bold">
                        Avg Latency
                      </div>
                    ...
                    <div className="bg-dark-bg/60 p-2.5 rounded-xl border border-dark-border/40">
                      <div className="text-sm font-black text-white">
                        {loadResults.successCount} / {loadResults.concurrency}
                      </div>
                      <div className="text-[8px] text-dark-muted uppercase font-bold">
                        Success Reqs
                      </div>
                ...
                <div className="h-28 bg-dark-bg/60 border border-dashed border-dark-border rounded-2xl flex items-center justify-center">
                  <div className="text-xs text-dark-muted animate-pulse">
                    Running database connections benchmark...
                  </div>
                </div>
              ) : (
                <div className="h-28 bg-dark-bg/60 border border-dashed border-dark-border rounded-2xl flex items-center justify-center">
                  <div className="text-xs text-dark-muted italic">
                    Click button above to benchmark endpoint.
                  </div>
                </div>
              )}
            </Card>
        ...
        <div className="space-y-8 animate-fadeIn">
            ...
            {[
              {
                ...
                value: '7 / 7 Active',
                desc: 'All compose nodes online',
                color: 'text-green-500',
              },
              {
                label: 'Kubernetes Pods',
                value: '5 Replica Pods',
                desc: 'Running on EKS cluster',
                color: 'text-blue-500',
              },
              {
                label: 'Nginx Proxy',
                value: 'Healthy (HTTP 200)',
                desc: 'Routing /api and web views',
                color: 'text-amber-500',
              },
              {
                label: 'Prometheus Target',
                value: '1 Node active',
                desc: 'Scraping metrics at 5s rate',
                color: 'text-green-500',
              },
            ].map((kpi, i) => (
          ...
          <Card className="p-5 border border-dark-border/40 bg-dark-surface/10 space-y-4">
            <div className="flex items-center gap-2 border-b border-dark-border pb-3">
              ...
              <h3 className="text-xs font-black uppercase tracking-wider text-dark-text">
                Production Optimizations Engine
              </h3>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <Card glass className="p-5 border border-dark-border/40 space-y-4">
              <div className="flex items-center gap-2 border-b border-dark-border pb-3">
                <ShieldCheck className="w-4 h-4 text-green-500" />
                <h3 className="text-xs font-black uppercase tracking-wider text-dark-text">
                  Terraform Provisioning Orchestrator
                </h3>
              </div>
              <p className="text-[11px] text-dark-muted">
                Provision virtual private clouds (VPC), RDS Postgres database engines, and AWS EKS
                clusters directly from declared HCL configurations.
              </p>
              ...
              <div className="bg-black/80 border border-dark-border rounded-2xl p-4 font-mono text-[9.5px] text-amber-500 h-60 overflow-y-auto scrollbar-none space-y-1.5">
                ...
                {terraformLogs.length === 0 && (
                  <div className="text-dark-muted italic">
                    Ready to plan infrastructure. Click button above.
                  </div>
                )}
              </div>
            </Card>
